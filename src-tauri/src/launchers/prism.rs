...
pub struct PrismDetector;

#[async_trait]
impl LauncherDetector for PrismDetector {
    ...
    async fn get_config_path(&self) -> Result<PathBuf, String> {
        #[cfg(target_os = "windows")]
        {
            ...
            let path = PathBuf::from(appdata).join("PrismLauncher");
            if path.exists() {
                return Ok(path);
            }
            ...
            Err("Prism Launcher installation not found".to_string())
        }
        ...
        #[cfg(target_os = "macos")]
        {
            ...
            let path = PathBuf::from(home).join("Library/Application Support/PrismLauncher");
            if path.exists() {
                Ok(path)
            } else {
                Err("Prism Launcher config directory not found".to_string())
            }
        }
        ...
        #[cfg(target_os = "linux")]
        {
            ...
            if let Ok(xdg_data) = std::env::var("XDG_DATA_HOME") {
                let path = PathBuf::from(xdg_data).join("PrismLauncher");
                if path.exists() {
                    return Ok(path);
                }
            }
            
            let path = PathBuf::from(home).join(".local/share/PrismLauncher");
            if path.exists() {
                return Ok(path);
            }
            
            // Check for portable installation
            let portable_path = PathBuf::from("/opt/prismlauncher");
    ...
    async fn detect_instances(&self) -> Result<Vec<ExternalInstance>, String> {
        ...
        let mut entries = fs::read_dir(&instances_path)
            .await
            .map_err(|e| format!("Failed to read Prism Launcher instances directory: {}", e))?;
            ...
            if entry.file_type().await.map_or(false, |ft| ft.is_dir()) {
                ...
                if instance_cfg.exists() {
                    match self.parse_instance_config(&instance_dir, &instance_cfg, &mmc_pack).await {
                        ...
                        Err(e) => {
                            eprintln!("Failed to parse Prism Launcher instance at {:?}: {}", instance_dir, e);
                        }
                    }
                }
            }
    ...
    async fn get_executable_path(&self) -> Result<Option<PathBuf>, String> {
        ...
        #[cfg(target_os = "macos")]
        {
            let path = PathBuf::from("/Applications/Prism Launcher.app/Contents/MacOS/prismlauncher");
            if path.exists() {
                return Ok(Some(path));
            }
        }
        ...
        #[cfg(target_os = "linux")]
        {
            let common_paths = [
                ...
                "/usr/bin/org.prismlauncher.PrismLauncher",
            ];
...
#[async_trait]
impl LauncherLauncher for PrismDetector {
    ...
    async fn launch_instance(&self, instance_id: &str, _instance_path: &str) -> Result<(), String> {
        let executable = self.get_executable_path().await?
            .ok_or("Prism Launcher executable not found")?;
        
        let actual_id = instance_id.strip_prefix("prism-").unwrap_or(instance_id);
        
        tokio::process::Command::new(executable)
            .args(&["--launch", actual_id])
            .spawn()
            .map_err(|e| format!("Failed to launch Prism Launcher instance: {}", e))?;
